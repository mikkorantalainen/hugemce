...
export const validateEditorLicenseKey = (editor: Editor): void => {
  ...
  if (!hasApiKey && (Type.isUndefined(licenseKey) || validateLicenseKey(licenseKey) === 'INVALID')) {
    ...
    console.warn('You are running HugeRTE in evaluation mode. Provide a valid license key or specify the license to \'GPL\' to agree to the open source license terms. https://www.tiny.cloud/');
  }
};

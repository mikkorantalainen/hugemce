...
### Fixed
- The `fontsizeinput` increase and decrease size buttons now work on HugeRTE mobile. #TINY-9725
- The HugeRTE editor toolbar is now accessible for all screen widths; it no longer collapses into an inaccessible vertical line when the screen is scrolled horizontally. #TINY-9646
- Reverted the changes made, in HugeRTE 6.4.0, to UI button colors in focus, active, and enabled states. #TINY-9176
...
### Added
- New `expand` function added to `hugerte.selection` which expands the selection around the nearest word. #TINY-9001
- New `expand` function added to `hugerte.dom.RangeUtils` to return a new range expanded around the nearest word. #TINY-9001
...
- New `getTransparentElements` function added to `hugerte.html.Schema` to return a map object of transparent HTML elements. #TINY-9172
...
- A double bottom border appeared on inline mode editor for the `hugerte-5` skin. #TINY-9108
...
- The global `hugerte.dom.styleSheetLoader` was not affected by the `content_css_cors` option. #TINY-6037
...
- Removed extra bottom padding in the context toolbar of the `hugerte-5` skin. #TINY-8980
...
- Nuget packages for .NET and .NET Core are now configured to copy HugeRTE into `/wwwroot/lib/` when HugeRTE is installed into a project. #TINY-8611
...
- Fixed a double border showing for the `hugerte-5` skin when using `toolbar_location: 'bottom'`. #TINY-8564
...
- New `FakeClipboard` API on the `hugerte` global. #TINY-8353
...
### Improved
- New default theme styling for HugeRTE 6 facelift with old skin available as `hugerte-5` and `hugerte-5-dark`. #TINY-8373
...
### Changed
- HugeRTE is now MIT licensed. #TINY-2316
- Moved the `paste` plugin's functionality to HugeRTE core. #TINY-8310
- The `paste_data_images` option now defaults to `true`. #TINY-8310
- Moved the `noneditable` plugin to HugeRTE core. #TINY-8311
- Renamed the `noneditable_noneditable_class` option to `noneditable_class`. #TINY-8311
- Renamed the `noneditable_editable_class` option to `editable_class`. #TINY-8311
- Moved the `textpattern` plugin to HugeRTE core. #TINY-8312
- Renamed the `textpattern_patterns` option to `text_patterns`. #TINY-8312
- Moved the `hr` plugin's functionality to HugeRTE core. #TINY-8313
- Moved the `print` plugin's functionality to HugeRTE core. #TINY-8314
...
- The `hugerte.settings` global property is no longer set upon initialization. #TINY-7359
...
- Renamed the `hugerte.Env.os.isOSX` API to `hugerte.Env.os.isMacOS`. #TINY-8175
- Renamed the `hugerte.Env.browser.isChrome` API to `hugerte.Env.browser.isChromium` to better reflect its functionality. #TINY-8300
...
- Removed the `hugerte.utils.Promise` API. #TINY-8241
- Removed the `toHex` function for the `DOMUtils` and `Styles` APIs. #TINY-8163
- Removed the `execCommand` handler function from the plugin and theme interfaces. #TINY-7829
- Removed the `editor.settings` property as it has been replaced by the new Options API. #TINY-8236
- Removed the `shortEnded` and `fixed` properties on `hugerte.html.Node` class. #TINY-8205
...
- Removed the deprecated `editor_deselector`, `editor_selector`, `elements`, `mode` and `types` legacy HugeRTE init options. #TINY-7822
...
- The `fire()` function of `hugerte.Editor`, `hugerte.dom.EventUtils`, `hugerte.dom.DOMUtils`, `hugerte.util.Observable` and `hugerte.util.EventDispatcher` has been deprecated and will be removed in the next major release. Use the `dispatch()` function instead. #TINY-8102
...
### Fixed
- Published HugeRTE types failed to compile in strict mode. #TINY-7915
...
- Fixed a regression in the `hugerte.create()` API that caused issues when multiple objects were created. #TINY-7358
- Fixed the `LineHeight` command causing the `change` event to be fired inconsistently. #TINY-7048

## 5.7.1 - 2021-03-17

### Fixed
- Fixed the `help` dialog incorrectly linking to the changelog of HugeRTE 4 instead of HugeRTE 5. #TINY-7031
...
- Fixed an issue with external styles bleeding into HugeRTE. #TINY-6735
...
- Fixed dialog block messages were not passed through HugeRTE's translation system. #TINY-6971
...
### Added
- Added a TypeScript declaration file to the bundle output for HugeRTE core. #TINY-3785
...
- Added new `hugerte.dom.TextSeeker` API to allow searching text across different DOM nodes. #TINY-4200
...
- Fixed a type error introduced in HugeRTE version 5.0.2 when calling `editor.getContent()` with nested bookmarks. #TINY-3400
...
Full documentation for the version 5 features and changes is available at https://www.tiny.cloud/docs/hugerte/5/release-notes/release-notes50/
...
- Fixed the legacyoutput plugin to be compatible with HugeRTE 5.0. #TINY-2301
...
Initial list of features and changes is available at https://www.tiny.cloud/docs/hugerte/5/release-notes/release-notes50/.
...
- Fixed a bug where the "Powered by HugeRTE" branding link would break on XHTML pages. Patch contributed by tistre.
...
- Fixed a bug where HugeRTE didn't work with module loaders like webpack.
...
- Fixed bug where an inline HugeRTE editor initialized on a table did not set selection on load in Chrome.
...
- Fixed bug where the compat3x plugin wasn't working since the global hugerte references wasn't resolved correctly.
...
- Fixed bug where HugeRTE could not be lazyloaded through jquery integration.
...
### Removed
- Dropped support for IE 8-10 due to market share and lack of support from Microsoft. See hugerte docs for details.
...
- Fixed so hugerte global gets properly exported as a module when used with some module bundlers.
...
- Added new Promise return value to hugerte.init makes it easier to handle initialization.
...
- Added new beforeInitialize event to hugerte.util.XHR lets you modify XHR properties before open. Patch contributed by Brent Clintel.
...
- Fixed bug where the contextual table toolbar would appear incorrectly if HugeRTE was initialized inline inside a table.
...
### Fixed
- Fixed bug where blob uris not produced by hugerte would produce HTML invalid markup.
...
- Added new promise feature fill as hugerte.util.Promise.
...
### Added
- Added event binding logic to hugerte.util.XHR making it possible to override headers and settings before any request is made.
...
- Fixed bug where hugerte.remove with a selector not matching any editors would remove all editors.
...
- Fixed so the hugerte.dom.Sizzle is the latest version of sizzle this resolves the document context bug.
...
- Added new hugerte.util.Color class to handle color parsing and converting.
- Added new colorpicker UI widget element lets you add a hsv color picker to any form/window.
- Added new textpattern plugin that allows you to use markdown like text patterns to format contents.
- Added new resize helper element that shows the current width & height while resizing.
- Added new "once" method to Editor and EventDispatcher enables since callback execution events.
- Added new jQuery like class under hugerte.dom.DomQuery it's exposed on editor instances (editor.$) and globally under (hugerte.$).
